...
const handleOpenAIRequest = async (req, res) => {
  ...
  if (responseContent) {
    ...
    console.log(colors.green(`Response: ${responseContent}`));
  } else {
    res
      .status(500)
      .send(
        "Failed to get a valid response from OpenAI API after multiple attempts."
      );
  }
};
